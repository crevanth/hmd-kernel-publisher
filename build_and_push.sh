...
if [ -z "$GH_TOKEN" ]; then
    echo "Error: GitHub token (first argument) is empty." >&2
    exit 1
fi
...
# --- THIS IS THE FIX ---
# Configure Git to use gh as its credential helper.
...
# ... (The rest of the script is exactly the same and will now work) ...
