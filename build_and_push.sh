...
# ... (All other helper functions from the previous script remain the same) ...
...
if ! gh repo view "$GITHUB_REPO_URL" >/dev/null 2>&1; then
  ...
  gh repo create "$GITHUB_REPO_URL" \
    --public \
    --description "Kernel source history for the ${DEVICE_HUMAN}"
...
# ... (rest of the local repo prep is the same) ...
...
echo "$JSON_DATA" | jq -r --arg device "$DEVICE_HUMAN" '.[$device][] | "\(.name) \(.link)"' | while read -r ARCHIVE_NAME URL; do
    TAG="${ARCHIVE_NAME%.tar.*}"
    # Check if tag exists locally first
    if git rev-parse -q --verify "refs/tags/$TAG" >/dev/null 2>&1; then
        echo "==> SKIP $TAG (tag exists locally)"
        continue
    fi
    # ... (rest of the main loop is identical to the previous script) ...
    echo "==> Processing $TAG"
...
# Check if remote 'origin' exists, if not add it. Otherwise, set the URL to be sure.
