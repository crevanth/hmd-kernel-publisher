...
# --- NEW FUNCTION TO GET COMMIT DATE ---
# Fetches the Last-Modified header from the URL for an accurate timestamp.
# Falls back to scanning local files if the header is unavailable.
get_commit_date() {
  local url="$1"
  local fallback_dir="$2"
  
  # Fetch only the headers and grep for Last-Modified, case-insensitive
  local http_date_str
  http_date_str=$(curl -sI "$url" | grep -i "^Last-Modified:" | sed 's/Last-Modified: //i' | tr -d '\r\n')

  if [[ -n "$http_date_str" ]]; then
    # Attempt to parse the date string. If successful, return it.
    if parsed_date=$(date -u -d "$http_date_str" +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null); then
      echo "$parsed_date"
      return
    fi
  fi
  
  # If we're here, the header was missing or couldn't be parsed. Use the fallback.
  echo "-> Warning: Could not get valid Last-Modified header. Falling back to file modification time." >&2
  newest_iso_utc "$fallback_dir"
}
# --- END OF NEW FUNCTION ---

# --- Script Start & Validation ---
need git; need curl; need tar; need rsync; need jq; need gh
# ... (rest of validation and auth is the same) ...
...
# ... (Dynamic config, repo management, and local prep are the same) ...
...
mkdir -p "$REPO_DIR"; cd "$REPO_DIR"; if [ ! -d .git ]; then git init; fi; git checkout -B "$BRANCH_NAME"; grep -qxF ".DS_Store" .git/info/exclude 2>/dev/null || echo ".DS_Store" >> .git/info/exclude; grep -qxF ".cache_downloads/" .git/info/exclude 2>/dev/null || echo ".cache_downloads/" >> .git/info/exclude; CACHE_DIR="$PWD/.cache_downloads"; mkdir -p "$CACHE_DIR";
...
echo "$JSON_DATA" | jq -r --arg device "$DEVICE_HUMAN" '.[$device][] | "\(.name) \(.link)"' | while read -r ARCHIVE_NAME URL; do
    ...
    KFOLDER="$(basename "$KDIR")"; echo "-> Using detected kernel root: $KFOLDER"; clean_repo_root; rsync -a --exclude='.git' "$KDIR"/ "$PWD"/; rm -f ".release-info" ".DS_Store" || true;
    VERSION_PART=$(echo "$TAG" | sed -e "s/^${DEVICE_SLUG}_//i" -e "s/^nokia[0-9]*[a-z]*_//i");
    
    # --- THIS IS THE CHANGED LINE ---
    # Use the new function to get the commit date from the URL header
    COMMIT_DATE="$(get_commit_date "$URL" "$KDIR")"
    
    export GIT_AUTHOR_DATE="$COMMIT_DATE"; export GIT_COMMITTER_DATE="$COMMIT_DATE";
