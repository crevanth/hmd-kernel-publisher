...
The source code published by this tool is subject to the licenses included within the code itself (typically the GNU General Public License, version 2). **Users are solely responsible for ensuring their use of the code complies with these licenses.** The maintainers of this project provide this tool "as-is" without warranty and are not responsible for any misuse of the source code.

## How It Works

The core logic is handled by the `build_and_push.sh` script, which performs the following steps for each device:

1.  **Fetch Data**: It reads a list of devices and their corresponding kernel source archive URLs from the `hmd_versions.json` file.
...
The entire process is automated using GitHub Actions, defined in `.github/workflows/build_device_repo.yml`. The workflow can be triggered in two ways:

### 1. Scheduled Run
- **When**: Automatically runs once a week (every Sunday at 02:00 UTC).
- **What**: It fetches the complete list of devices from the JSON file and runs the `build_and_push.sh` script for every single one, ensuring all repositories are kept up-to-date with the latest releases.

### 2. Manual Trigger
- **When**: Can be triggered manually at any time from the GitHub Actions tab.
- **What**: This allows you to run the process for a single, specific device. This is useful for adding a new device or re-running the process for a device that may have failed previously.
...
-   **`build_and_push.sh`**:
    ...
    -   `JSON_REPO`: The repository that hosts the `hmd_versions.json` tracker file.
-   **GitHub Actions**:
...
The `build_and_push.sh` script requires the following command-line tools to be installed:
